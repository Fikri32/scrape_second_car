...
def scrape_price_category(price, base_url, headers, user_agents, dated_dir):
    ...
    if response:
        ...
        h1_element = soup.find("h1", class_="headline delta flush")
        ...
        if div:
            ...
            if data_smenu_params:
                ...
                if all_details:
                    csv_file_name = f"carmudi_{price_category}_{pendulum.now().format('YYYY-MM-DD')}.csv"
...
def carmudi_scrape():
    """Main function to start scraping process for Carmudi."""
    config_path = os.path.abspath(
        os.path.join(
            os.path.dirname(__file__), "../../../scrape/script/carmudi/config.json"
        )
    )
    config = load_config(config_path)

    now = pendulum.now().format("YYYY-MM-DD")
    output_base_dir = "/home/miracle/mobil/scrape/output/carmudi"
