...
input_path = f"/home/miracle/mobil/scrape/output/carmudi/{now}/"
...
df_carmudi = spark.read.csv(
    input_path + "*.csv", header=True, inferSchema=True, sep=","
)
df_carmudi = df_carmudi.withColumn("Price", col("Price").cast(IntegerType()))
df_carmudi = df_carmudi.withColumn("KM", col("KM").cast(IntegerType()))
df_carmudi = df_carmudi.withColumn("Seats", col("Seats").cast(IntegerType()))


# Convert Spark DataFrame to Pandas DataFrame
pandas_df = df_carmudi.toPandas()
